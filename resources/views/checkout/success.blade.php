...
<body>
  ...
  <div class="success-card">
    ...
    <p class="text-muted mb-4">
      Thank you, <strong>{{ $checkout->user->name ?? 'Customer' }}</strong>.<br>
      Your payment for <strong>{{ $checkout->meal->name }}</strong> has been received successfully.
    </p>

    {{-- Price Summary --}}
    <div class="price-summary mb-3 small">
      <p><span>Base Price</span> <strong>AED {{ number_format($basePrice, 2) }}</strong></p>
      <p><span>{{ $vatLabel }} ({{ number_format($vatPercent, 2) }}%)</span> <strong>AED {{ number_format($vatAmount, 2) }}</strong></p>
      <div class="divider"></div>
      <p class="fw-semibold text-success"><span>Total (Incl. VAT)</span> <strong>AED {{ number_format($total, 2) }}</strong></p>
    </div>
    ...
    <p class="text-muted small mb-0">
      A confirmation email will be sent to <strong>{{ $checkout->user->email }}</strong>.
    </p>
