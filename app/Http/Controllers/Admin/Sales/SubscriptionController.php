...
class SubscriptionController extends Controller
{
    ...
    public function retryRenewal(Request $request, Subscription $subscription, $renewalId)
    {
        ...
        try {
            $result = app(\App\Services\AutoChargeService::class)->attemptCharge($subscription);
    ...
    {
        try {
            // Prevent renew if already active & up to date
            ...
            // 1️⃣ Attempt Charge
            $result = app(\App\Services\AutoChargeService::class)->attemptCharge($subscription, $price, $tax);

            // 2️⃣ Create Renewal Log
            $subscription->renewalLogs()->create([
                'gateway_id'   => 1, // Stripe by default
                'reference'    => $result['reference'] ?? null,
                'amount'       => $price,
                'tax_amount'   => $result['tax_amount'] ?? 0,
                'total_amount' => $result['total'] ?? $price,
                'currency_id'  => 1, // Default AED, you can dynamic this
                'status'       => $result['status'] ?? 'pending',
                'message'      => 'Manual renewal by admin',
                'charged_at'   => now(),
            ]);

            // 3️⃣ Extend Subscription Dates
            $durationDays = $subscription->mealPackagePrice->duration;
            $newStartDate = now();
            $newEndDate   = now()->addDays($durationDays)->subDay();
